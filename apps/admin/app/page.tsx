import { InlineCode, P } from "@workspace/ui/components/text";
  ...
  return (
    <TableRow>
      {columns.map((column, key) => (
        <TableCell
          key={key}>{typeof column.value === "string" ? (container.data !== undefined ? container.data[column.value] : "") : column.value!(container.data, service)}</TableCell>
      ))}
    </TableRow>
  );
            ...
            <TableHeader>
              <TableRow>
                {columns.map((column, index) => (
                  <TableHead className={"font-bold"} key={`HeaderColumn-${index}`}>{column.display}</TableHead>
                ))}
              </TableRow>
            </TableHeader>
            <TableBody>
              <Container service={DockerService.LOGS} />
